...
export const initiateOnlinePayment = async (req, res) => {
  try {
    ...
    if (!studentId || !studentFeeDueId || !amount) {
      return res.status(400).json({ message: "Missing required fields: studentId, studentFeeDueId, amount." });
    }
    ...
    if (studentFeeDue.student.toString() !== studentId) {
      return res.status(403).json({ message: "Unauthorized: This fee does not belong to your account." });
    }

    // IMPORTANT: Validate amount against outstanding balance
    if (parsedAmount > studentFeeDue.balance) {
      return res.status(400).json({ message: `Amount exceeds outstanding balance of ₹${studentFeeDue.balance.toFixed(2)}.` });
    }
    ...
    const orderOptions = {
      ...
      notes: { // Custom notes that will be passed back in the webhook
...
export const handleRazorpayWebhook = async (req, res) => {
  ...
  if (digest === req.headers['x-razorpay-signature']) {
    ...
    try {
      switch (event) {
        case 'payment.captured': // Payment was successful
          ...
          if (!payment) {
            console.warn(`Payment record not found or already processed for gateway order ID: ${gatewayOrderId}.`);
            // This can happen if webhook is sent multiple times or already updated.
            return res.status(200).json({ message: 'Payment record not found or already processed.' });
          }
          ...
          if (studentFeeDue) {
            ...
            console.log(`Payment captured for order ${gatewayOrderId}. Student Fee Due and Student records updated.`);
          } else {
            console.error(`CRITICAL: StudentFeeDue not found for payment ID: ${payment._id}. Data inconsistency!`);
        ...
        case 'payment.failed': // Payment failed
          ...
          if (failedPayment) {
            console.log(`Payment failed for order ${failedGatewayOrderId}. Payment record status updated.`);
          } else {
            console.warn(`Failed payment record not found or already processed for gateway order ID: ${failedGatewayOrderId}.`);
          }
          break;
    ...
    console.warn('Razorpay webhook signature mismatch! Possible malicious request or misconfiguration.');
